...
@pytest.mark.smoke
@pytest.mark.description("Verify agency can log in with valid credentials")
def test_login_valid_credentials(login):
    page = login
    login_page = LoginPage(page)
    assert login_page.is_logged_in(), f"Expected to be logged in, current URL: {page.url}"
...
def test_login_invalid_credentials(setup_browser, config):
    ...
    login_page = LoginPage(page)
    login_page.login_with_invalid_credentials("wrong@agency.com", "wrongpass")
