...
def test_login_invalid_username(setup_browser, config):
    ...
    assert error_message == "Please check your username and password. If you still can't log in, contact your Salesforce administrator.", f"Expected error message, but got: {error_message}"
...
def test_login_empty_password(setup_browser, config):
    ...
    # Verify that the error message is shown
    error_message = login_page.get_error_message()
    assert error_message == "Please enter your password.", f"Expected error message, but got: {error_message}"
    print(f"Error message displayed: {error_message}")